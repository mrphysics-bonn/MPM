...
def create_batchFile(path, nTE):
    ...
    fhead = os.path.join(path, 'nii','RF_sens_head','*-1.nii')
    fhead = glob.glob(fhead)
    fbody = os.path.join(path, 'nii','RF_sens_body','*-1.nii')
    fbody = glob.glob(fbody)
    
    #RF receive setting
    f.write("matlabbatch{2}.spm.tools.hmri.create_mpm.subj.sensitivity.RF_once = {\n")
    f.write("                                                                     '%s,1'\n"%fhead[0])
    f.write("                                                                     '%s,1'\n"%fbody[0])
    f.write("                                                                     };\n")
